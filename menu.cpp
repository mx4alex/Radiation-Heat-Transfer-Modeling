...
static const int g_maxSteps= 2000;
static int   g_stepCount=0;
static double g_alpha= 0.0005; 
static double g_kappa= 0.25;  
int g_dt= 300;                

static RadiationModel g_model= RadiationModel::S2S;
    ...
    if(appState==AppState::MENU){
        ...
        if(ImGui::CollapsingHeader("Add Object", ImGuiTreeNodeFlags_DefaultOpen)){
            ...
            if(ImGui::Button("Add Object##button")){
                ...
                if(!loadASCIISTL(stlFile, vertices, faces)){
                    ...
                    newSphere.vertices= std::move(vertices);
                    newSphere.faces= std::move(faces);
                    for(auto &fc: newSphere.faces){
                        fc.temp= initTemp;
                    }
                    double totalA=0.;
                    for(auto &fc: newSphere.faces){
                    ...
                    newSphere.totalArea= totalA;
                    newSphere.mass= sphereMass;
                    newSphere.cp  = sphereCp;
                    for(int b=0; b<3; b++){
                        newSphere.reflectivity[b]= reflectCoeff;
                        newSphere.emissivity[b]  = absorbCoeff;
                    }
                    newSphere.boundaryCondition= (bcTypeIdx==0)?BoundaryConditionType::First:BoundaryConditionType::Second;
                    newSphere.boundaryValue= bcValue;
                    newSphere.stlFileName= stlFile;
                    newSphere.position= {pos[0], pos[1], pos[2]};
                    newSphere.initialTemperature= initTemp;

                    g_spheres.push_back(std::move(newSphere));
                    updateGlobalData();

                    strcpy(stlFile,"sphere_ascii.stl");
        ...
        int sphereToRemove=-1;
        if(ImGui::CollapsingHeader("Objects", ImGuiTreeNodeFlags_DefaultOpen)){
            for(int i=0; i<(int)g_spheres.size(); i++){
                ImGui::PushID(i);
                if(ImGui::TreeNode(("Object "+std::to_string(i+1)).c_str())){
                    ImGui::BeginChild("ObjectInfo", ImVec2(0,200), true);
                    ImGui::BeginDisabled(true);

                    {
                        char buf[256];
                        snprintf(buf, sizeof(buf), "%s", g_spheres[i].stlFileName.c_str());
                        ImGui::InputText("STL File", buf, sizeof(buf));
                    }
                    {
                        char posBuffer[128];
                        snprintf(posBuffer,sizeof(posBuffer),"%.2f, %.2f, %.2f",
                                 g_spheres[i].position.x,
                                 g_spheres[i].position.y,
                                 g_spheres[i].position.z);
                        ImGui::InputText("Position", posBuffer, sizeof(posBuffer));
                    }
                    {
                        char tempBuffer[64];
                        snprintf(tempBuffer,sizeof(tempBuffer),"%.2f", g_spheres[i].initialTemperature);
                        ImGui::InputText("Initial Temp", tempBuffer, sizeof(tempBuffer));
                    }
                    {
                        const char* bcItems[]={"1","2"};
                        ImGui::Text("Boundary Condition: %s", bcItems[(int)g_spheres[i].boundaryCondition]);
                        if(g_spheres[i].boundaryCondition==BoundaryConditionType::Second){
                            char bValueBuffer[64];
                            snprintf(bValueBuffer,sizeof(bValueBuffer),"%.6f", g_spheres[i].boundaryValue);
                            ImGui::InputText("Heat Flux (W/m²)", bValueBuffer, sizeof(bValueBuffer));
                        }
                        char massBuf[64];
                        snprintf(massBuf,sizeof(massBuf),"%.6f", g_spheres[i].mass);
                        ImGui::InputText("Mass (kg)", massBuf, sizeof(massBuf));
                        char cpBuf[64];
                        snprintf(cpBuf,sizeof(cpBuf),"%.6f", g_spheres[i].cp);
                        ImGui::InputText("Specific Heat (J/kg*K)", cpBuf, sizeof(cpBuf));
                        char areaBuf[64];
                        snprintf(areaBuf,sizeof(areaBuf),"%.6f", g_spheres[i].totalArea);
                        ImGui::InputText("Total Area (m^2)", areaBuf, sizeof(areaBuf));
                    }
                    {
                        char reflBuf[64];
                        snprintf(reflBuf,sizeof(reflBuf),"%.3f", g_spheres[i].reflectivity[0]);
                        ImGui::InputText("Reflection Coefficient", reflBuf, sizeof(reflBuf));
                        char absorpBuf[64];
                        snprintf(absorpBuf,sizeof(absorpBuf),"%.3f", g_spheres[i].emissivity[0]);
            ...
            if(sphereToRemove>=0){
                g_spheres.erase(g_spheres.begin()+ sphereToRemove);
                updateGlobalData();
            }
        }
        ...
        if(ImGui::Button("Start Simulation", ImVec2(200,50))){
            if(g_spheres.empty()){
                std::cerr<<"No spheres defined!\n";
            } else {
                computeAllViewFactors(g_model);
                    ...
                    while(simulationRunning && g_stepCount<g_maxSteps){
                        {
                            ...
                            radiationOneStep(g_alpha);
                            conductionOneStepAll(g_kappa);
...
int main(int argc, char** argv){
    ...
    ImGuiIO &io= ImGui::GetIO();
    io.Fonts->Clear();
    ImFont* customFont= io.Fonts->AddFontFromFileTTF("Roboto-Medium.ttf",18.f);
    if(!customFont){
