...
double currentMinTemp = 200.0;
double currentMaxTemp = 2000.0;
...
void updateMinMaxTemps(){
    ...
    for(auto &gf: g_allFaces){
        double T = g_spheres[gf.sphereIndex].faces[gf.faceIndex].temp;
        if(T<mi) mi=T;
        if(T>ma) ma=T;
    }
    double pad=50.;
    currentMinTemp= std::max(200., mi-pad);
    currentMaxTemp= std::min(2000.,ma+pad);
}

void renderAllFaces(double Tmin, double Tmax){
    glBegin(GL_TRIANGLES);
    for(auto &gf: g_allFaces){
        int si = gf.sphereIndex;
        int fi = gf.faceIndex;
